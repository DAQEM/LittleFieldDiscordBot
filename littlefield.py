...
class Littlefield:
    def __init__(self, team_id='', password='', institution=''):
    ...
    def live_day(self):
        cash_bytes = self._get_content('LTStatus').splitlines()[-8].split()[-1]
        cash_str = cash_bytes.decode('utf-8')
        cash_numeric = ''.join(char for char in cash_str if char.isdigit() or char == ',')
        return cash_numeric.replace(',', '')

    def live_cash(self):
        cash_bytes = self._get_content('LTStatus').splitlines()[-6].split()[-1]
        cash_str = cash_bytes.decode('utf-8')
        cash_numeric = ''.join(char for char in cash_str if char.isdigit() or char == ',')
        return cash_numeric.replace(',', '')
    ...
    def _get_session_id(self):
        headers = {'User-Agent': 'PostmanRuntime/7.36.3', 'Content-Type': 'application/x-www-form-urlencoded'}
        payload = f"institution={self.institution}&ismobile=false&id={self.team_id}&password={self.password}"
    ...
    def _get_content(self, path, params=None):
        cookie = {'JSESSIONID': self.session_id}
        r = requests.get('http://op.responsive.net/Littlefield/' + path, cookies=cookie, params=params)
        return r.content
